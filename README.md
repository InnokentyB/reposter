...
- Threads media теперь умеет baseline path через локальное сохранение файла и публичный `MEDIA_BASE_URL`
...
- `MEDIA_STORAGE_PATH`
- `MEDIA_BASE_URL`: нужен для Threads media, должен указывать на публично доступный URL-префикс
- `VK_COMMUNITY_ID`
- `VK_ACCESS_TOKEN`
- `OK_GROUP_ID`
- `OK_ACCESS_TOKEN`
...
- Threads photo posts теперь работают через [media_store.py](/Users/innokentyb/Documents/Repost%20bot/repost_bot/media_store.py), если задан публичный `MEDIA_BASE_URL`
...
- OK autopost пока не доведен до production flow

Чтобы Threads media реально работал, каталог из `MEDIA_STORAGE_PATH` должен отдаваться наружу по URL из `MEDIA_BASE_URL`.
